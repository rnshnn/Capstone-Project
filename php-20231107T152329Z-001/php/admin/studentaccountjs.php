...
$(document).ready(function() {
    ...
    $("#addField").click(function() {
        ...
        var newRow = '<div class="form-row add_one">' +
            ...
            '][R_FIRST]" placeholder="Enter name">' +
            '</div>' +
            '<div class="form-group col-md-2">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            ...
            '<div class="form-group col-md-2">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            '][R_COU]" placeholder="Enter Course">' +
            '</div>' +
            '<div class="form-group col-md-2">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            '][R_YEAR]" placeholder="Enter Year">' +
            '</div>' +
            '<div class="form-group col-md-2" style="display:none">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            '][R_STATUS]" id="R_STATUS" value="Not Active">' +
            '</div>' +
            '<div class="form-group col-md-2" style="display:none">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            '][R_RORA]" id="R_RORA" value="approve">' +
            '</div>' +
            '<div class="form-group col-md-2" style="display:none">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            '][R_VERIFIED]" id="R_VERIFIED" value="1">' +
            '</div>' +
            '<div class="form-group col-md-2" style="display:none">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            '][R_STU_POS]" id="R_STU_POS" value="Student">' +
            '</div>' +
            '<div class="form-group col-md-2" style="display:none">' +
            '    <input type="text" class="form-control" name="accounts[' + accountCount +
            '][R_SMS]" value="Enable">' +
    ...
    $("#registerButton").click(function(event) {
        ...
        const requiredFields = [
            "accounts[0][R_FIRST]", "accounts[0][R_STU]", "accounts[0][R_EMAIL]",
            "accounts[0][R_PASS]", "accounts[0][R_COU]", "accounts[0][R_YEAR]",
            "accounts[" + accountCount + "][R_FIRST]", "accounts[" + accountCount + "][R_STU]",
            "accounts[" + accountCount + "][R_EMAIL]", "accounts[" + accountCount + "][R_PASS]",
            "accounts[" + accountCount + "][R_COU]", "accounts[" + accountCount + "][R_YEAR]"
        ];
...
userPosElements.forEach(element => {
    ...
    element.style.color = (pos === 'Alumni') ? 'orange' : ((pos === 'Student') ? 'darkblue' : (pos ===
