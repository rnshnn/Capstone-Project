...
$sql = "UPDATE $table SET S_STA_STU = 'seen' WHERE ";
...
$userEmail = $_SESSION['R_EMAIL']; // Use the session variable
$emailField = getEmailField($table); // Get the correct email field based on the table

$sql = "SELECT * FROM $table WHERE S_STA_STU = 'unseen' AND $emailField = ?";
$stmt = mysqli_prepare($conn, $sql);
...
switch ($table) {
    ...
    case 'done':
        return 'D_EMAIL';
    default:
        return 'S_EMAIL'; // Default to S_EMAIL for unknown tables
}
}
                    ...
                    <div class="notification-popup" id="notificationPopup">
                        ...
                        function renderNotifications($notifications, $table) {
                            foreach ($notifications as $notification) {
                                $statusClass = ($notification["S_STA_STU"] == "seen") ? "read" : "";
                        ...
                        function getNotificationLink($table) {
                            switch ($table) {
                                case 'request':
                                    return 'http://localhost/newventure/student/pending';
                                case 'payment':
                                    return 'http://localhost/newventure/student/payment';
                                case 'process':
                                    return 'http://localhost/newventure/student/onprocess';
                                case 'releasing':
                                    return 'http://localhost/newventure/student/releasing';
                                case 'done':
                                    return 'http://localhost/newventure/student/done';
                                default:
                                    return '#'; 
                            }
                        }
...
function markNotificationAsSeen(id, table) {
    ...
    switch (table) {
        case 'request':
            urls.push("payment.php", "pending.php", "payment.php", "process.php", "releasing.php", "done.php",
                "dashboard.php");
            break;
        case 'payment':
            urls.push("payment.php", "pending.php", "payment.php", "process.php", "releasing.php", "done.php",
...
function updateNotificationCount() {
    var unseenCount = notifications.filter(function(notification) {
        return notification.S_STA_STU === "unseen";
    }).length;
