    ...
    <div style="flex: 1; display: flex;">
        ...
        <main role="main" class="col-md-9 ml-md-auto col-lg-10 px-1" id="main-content">
            ...
            <div class="container-fluid">
                ...
                $query = "SELECT * FROM done WHERE D_EMAIL = '$registrar_email'";
                ...
                ?>
                    ...
                    <table class="table table-bordered table-striped table-hover table table-sm">
                        ...
                        if (isset($_GET['search'])) {
                            ...
                            $query = "SELECT COUNT(*) as total FROM DONE WHERE D_EMAIL=? AND CONCAT(D_FULL,D_CODE,D_documentType) LIKE ?";
                            $stmt = $conn->prepare($query);
                            $stmt->bind_param("ss", $registrar_name, $filtervalues);
                        } else {
                            $query = "SELECT COUNT(*) as total FROM DONE WHERE D_EMAIL=?";
                            $stmt = $conn->prepare($query);
                        ...
                        if (isset($_GET['search'])) {
                            $filtervalues = '%' . $_GET['search'] . '%'; // Add % wildcards
                            $query = "SELECT * FROM DONE WHERE D_EMAIL=? AND CONCAT(D_FULL,D_CODE,D_documentType) LIKE ? LIMIT $results_per_page OFFSET $offset";
                            $stmt = $conn->prepare($query);
                            $stmt->bind_param("ss", $registrar_name, $filtervalues);
                        } else {
                            $query = "SELECT * FROM DONE WHERE D_EMAIL=? LIMIT $results_per_page OFFSET $offset";
                            $stmt = $conn->prepare($query);
                            $stmt->bind_param("s", $registrar_name);
                        }
                        $stmt->execute();
