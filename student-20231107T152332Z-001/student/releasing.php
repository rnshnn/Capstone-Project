...
<head>
    ...
    <title>RMS - Student releasing</title>
</head>
...
<body style="display: flex; flex-direction: column; min-height: 100vh;">
    ...
    <nav class="col-md-3 col-lg-2 d-md-block bg-light border-right" id="sidebar">
            ...
            <div class="list-group list-group-flush">
                ...
                <a href="Releasing/<?= urlencode($registrar_name) ?>" class="list-group-item list-group-item-action active">
        ...
        <button id="sidebarToggle" class="btn btn-secondary btn-notification position-fixed"
            ...
            <div class="container-fluid">
                ...
                $query = "SELECT * FROM releasing WHERE L_EMAIL = '$registrar_email'";
                ...
                ?>
                    ...
                    <table class="table table-bordered table-striped table-hover table table-sm">
                        ...
                        if (isset($_GET['search'])) {
                            ...
                            $query = "SELECT COUNT(*) as total FROM releasing WHERE L_EMAIL=? AND CONCAT(L_FULL,L_CODE,L_documentType) LIKE ?";
                            $stmt = $conn->prepare($query);
                            $stmt->bind_param("ss", $registrar_name, $filtervalues);
                        } else {
                            $query = "SELECT COUNT(*) as total FROM releasing WHERE L_EMAIL=?";
                            $stmt = $conn->prepare($query);
                        ...
                        if (isset($_GET['search'])) {
                            $filtervalues = '%' . $_GET['search'] . '%'; // Add % wildcards
                            $query = "SELECT * FROM releasing WHERE L_EMAIL=? AND CONCAT(L_FULL,L_CODE,L_documentType) LIKE ? LIMIT $results_per_page OFFSET $offset";
                            $stmt = $conn->prepare($query);
                            $stmt->bind_param("ss", $registrar_name, $filtervalues);
                        } else {
                            $query = "SELECT * FROM releasing WHERE L_EMAIL=? LIMIT $results_per_page OFFSET $offset";
                            $stmt = $conn->prepare($query);
                            $stmt->bind_param("s", $registrar_name);
                        }
                        $stmt->execute();
                        ...
                        ?>
                            ...
                            if (messageContent_<?= $i; ?>.toLowerCase() === 'ready to pick up' ||
                                messageContent_<?= $i; ?>.toLowerCase() === 'ready to ship out') {
