    ...
    <div style="flex: 1; display: flex;">
        <nav class="col-md-3 col-lg-2 d-md-block bg-light border-right" id="sidebar">
            ...
            <div class="list-group list-group-flush">
                ...
                <a href="releasing/<?= urlencode($registrar_name) ?>"
        ...
        <main role="main" class="col-md-9 ml-md-auto col-lg-10 px-1">
            <div class="container-fluid">
                <div class="mt-5">
                    <h3 class="mb-4"> <i class="fas fa-hourglass-start mr-2"></i> Releasing</h3>

                    <?php

                    $query = "SELECT COUNT(*) AS total FROM releasing"; // Count all rows in the table
                    ...
                    if ($query_run) {
                        ...
                        echo '<h6>You have (' . $row_count . ') Releasing request</h6>';
                    } else {
                ...
                ?>
                    ...
                    if (isset($_GET['search'])) {
                        ...
                        $query = "SELECT * FROM releasing WHERE L_FULL LIKE '%$search%' OR L_documentType LIKE '%$search%' OR L_CODE LIKE '%$search%'";
                    } else {
                        // If no search query provided, use the original SQL query
                        $query = "SELECT * FROM releasing LIMIT $results_per_page OFFSET $offset";
                    }

                    // Get total number of results (without LIMIT)
                    $total_results_query = "SELECT COUNT(*) as total FROM releasing";
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($users = mysqli_fetch_assoc($result)) {
                            ...
                            <tr data-toggle="modal" data-target="#recordModal<?= $i; ?>">
                                ...
                                <td><?= $users['L_COU'] ?></td>
                                <td><?= $users['L_YEAR'] ?></td>
                                <td><?= $users['L_documentType'] ?> <br> <?= $users['L_documentType_2'] ?> <br> <?= $users['L_documentType_3'] ?></td>
                                <td><?= $users['L_numCopies'] ?> <br> <?= $users['L_numCopies_2'] ?> <br> <?= $users['L_numCopies_3'] ?></td>
                                <td><?= $users['L_price'] ?></td>
                                <td><?= $users['L_CODE'] ?></td>
                                <td><?= $users['L_DEL'] ?></td>
                                <td id="S_MES_<?= $i; ?>"><?= $users['S_MES']; ?></td>
                                <td><?= $users['L_DATE'] ?></td>
                                <td><?= $users['L_DATE_DONE'] ?></td>

                            </tr>
                            <?php include "../php/registrar/releasingjs.php"; ?>
                            <?php
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p><strong>Name:</strong> <?= $users['L_FULL']; ?></p>
                                <p><strong>Student ID:</strong> <?= $users['L_SID']; ?></p>
                                <p><strong>Course:</strong> <?= $users['L_COU']; ?></p>
                                <p><strong>Year:</strong> <?= $users['L_YEAR']; ?></p>
                                <p><strong>If under Graduation:</strong> <?= $users['L_UND']; ?></p>
                                <p><strong>Company:</strong> <?= $users['L_COM']; ?></p>
                                <p><strong>Position:</strong> <?= $users['L_POS']; ?></p>
                            </div>
                            <div class="col-md-6">
                            <p><strong>Request (1):</strong> <?= $users['L_documentType']; ?> -  <?= $users['L_numCopies']; ?></p>
                                <p><strong>Request (2):</strong> <?= $users['L_documentType_2']; ?> -  <?= $users['L_numCopies_2']; ?></p>
                                <p><strong>Request (3):</strong> <?= $users['L_documentType_3']; ?> -  <?= $users['L_numCopies_3']; ?></p>
                                <p><strong>First request:</strong> <?= $users['L_firstRequest']; ?></p>
                                <p><strong>Price:</strong> <?= $users['L_price']; ?></p>
                                <p><strong>Reference No:</strong> <?= $users['L_CODE']; ?></p>
                                <p><strong>Date of request:</strong> <?= $users['L_DATE']; ?></p>
                            </div>
                        </div>

                        <div class="mt-4">
                            <div class="d-flex justify-content-center mt-2" style="width: 100%;">
                                <form action="../php/registrar/releasingapprove.php" method="post"
                                    id="approveForm<?= $i; ?>" style="width: 100%; margin-top:10px;">
                                    <input type="hidden" name="release" value="<?= $users['L_ID']; ?>">
                                    <input type="hidden" name="L_DEL" value="<?= $users['L_DEL']; ?>">
                                    <input type="hidden" name="L_REA" value="<?php echo htmlspecialchars($_SESSION['R_FULL']); ?>">
