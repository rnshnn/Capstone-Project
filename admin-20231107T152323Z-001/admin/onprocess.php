...
<head>
    ...
    <title>RMS - Admin on-process</title>
</head>
...
<body style="display: flex; flex-direction: column; min-height: 100vh;">
    ...
    <div style="flex: 1; display: flex;">
        <nav class="col-md-3 col-lg-2 d-md-block bg-light border-right" id="sidebar">
            ...
            <div class="list-group list-group-flush">
                ...
                <a href="Onprocess" class="list-group-item list-group-item-action active">
        ...
        <main role="main" class="col-md-9 ml-md-auto col-lg-10 px-1">
            <div class="container-fluid">
                <div class="mt-5">
                    <h3 class="mb-4"> <i class="fas fa-cogs mr-2"></i> On Process Request</h3>

                    <?php

                    $query = "SELECT COUNT(*) AS total FROM process"; // Count all rows in the table
                    ...
                    if ($query_run) {
                        ...
                        echo '<h6>You have (' . $row_count . ') On-process request</h6>';
                    } else {
                ...
                ?>
                    ...
                    <div class="input-group mb-3">
                        ...
                        <form action="" method="GET" class="w-50 d-flex align-items-center">
                            ...
                            <select type="text" name="search" class="form-control flex-grow-1 mr-1"
                                ...
                                <?php
                                $query = "SELECT DISTINCT O_ASIG FROM process";
                                $result = mysqli_query($conn, $query);

                                if ($result) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $O_ASIG = $row['O_ASIG'];
                                        echo "<option value='$O_ASIG'>$O_ASIG</option>";
                                    }
                                } else {
                        ...
                        <form action="../php/admin/process_assign" method="POST"
                            class="d-flex align-items-center ml-auto" id="form_process">
                            <select name="O_ASIG" class="form-control flex-grow-1 mr-1">
                            ...
                            <button class="btn btn-primary mr-4" type="button" id="process_assign">Confirm</button>
                        </form>



                        <form action="../php/admin/process_report" method="POST" id="form_processing"
                            class="d-flex align-items-center">
                            <input type="hidden" name="btn_clicked" id="btn_clicked" value="0">
                            <button class="btn btn-info" type="button" id="btn_process">Generate Report</button>
                        </form>
                    ...
                    if (isset($_GET['search'])) {
                        ...
                        $query = "SELECT * FROM process WHERE O_ASIG LIKE '%$search%'";
                    } else {
                        // If no search query provided, use the original SQL query
                        $query = "SELECT * FROM process LIMIT $results_per_page OFFSET $offset";
                    }

                    // Get total number of results (without LIMIT)
                    $total_results_query = "SELECT COUNT(*) as total FROM process";
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($users = mysqli_fetch_assoc($result)) {
                            ...
                            <tr data-toggle="modal" data-target="#recordModal<?= $i; ?>">
                                ...
                                <td><?= $users['O_price'] ?></td>
                                <td><?= $users['O_CODE'] ?></td>
                                <td style="color: darkgreen;"><?= $users['O_ASIG'] ?></td>
    ...
    <?php include "../php/admin/processjs.php"; ?>
