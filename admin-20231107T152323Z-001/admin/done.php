...
<head>
    ...
    <title>RMS - Admin done request</title>
</head>
    ...
    <div style="flex: 1; display: flex;">
        <nav class="col-md-3 col-lg-2 d-md-block bg-light border-right" id="sidebar">
            ...
            <div class="list-group list-group-flush">
                ...
                <a href="done" class="list-group-item list-group-item-action active">
        ...
        <main role="main" class="col-md-9 ml-md-auto col-lg-10 px-1">
            <div class="container-fluid">
                <div class="mt-5">
                    <h3 class="mb-4"> <i class="fas fa-check-circle mr-2"></i> Done Request</h3>
                ...
                ?>
                    ...
                    <div class="input-group mb-3">
                        <form action="" method="GET" class="w-50 d-flex align-items-center">
                            ...
                            <select type="text" name="search" class="form-control flex-grow-1 mr-1"
                                ...
                                <?php
                                $query = "SELECT DISTINCT D_ASIG FROM done";
                                $result = mysqli_query($conn, $query);

                                if ($result) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $D_ASIG = $row['D_ASIG'];
                                        echo "<option value='$D_ASIG'>$D_ASIG</option>";
                                    }
                                } else {
                        ...
                        <form action="../php/admin/done_download_delete" method="POST"
                        ...
                        <form action="../php/admin/done_report.php" method="POST" id="form_release"
                    ...
                    if (isset($_GET['search'])) {
                        ...
                        $query = "SELECT * FROM done WHERE D_ASIG LIKE '%$search%'";
                    } else {
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($users = mysqli_fetch_assoc($result)) {
                            ...
                            <tr data-toggle="modal" data-target="#recordModal<?= $i; ?>">
                                ...
                                <td><?= $users['D_COU'] ?></td>
                                <td><?= $users['D_YEAR'] ?></td>
                                <td><?= $users['D_documentType'] ?> <br> <?= $users['D_documentType_2'] ?> <br>
                                    <?= $users['D_documentType_3'] ?></td>
                                <td><?= $users['D_numCopies'] ?> <br> <?= $users['D_numCopies_2'] ?> <br>
                                    <?= $users['D_numCopies_3'] ?></td>
                                <td><?= $users['D_price'] ?></td>
                                <td><?= $users['D_CODE'] ?></td>
                                <td style="color: darkgreen;"><?= $users['D_ASIG'] ?></td>
                                <td style="color: darkgreen;"><?= $users['D_REA'] ?></td>
